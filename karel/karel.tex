...
Otherwise we have not changed Pattis' original ideas significantly.
...
\end{itemize}
By {\em recursion} we mean that a command makes a call to itself.
...
We have just seen one important fact: To prevent infinite recursion, one always needs {\tt if} or {\tt if-else} 
statement of some sort where one branch makes a recursive call, and the other branch does not. The branch without a recursive 
call is called the {\em base case}. A bad example of recursive command without a base case would be 
...
\section{Section H - Logical Variables}
