...
not needed for that. Thanks to the beautiful simplicity of Karel's language, 
the resulting computer codes are crystal clear.
...
learning language-specific details, because the most important skill in 
computer programming -- designing great algorithms -- you will already have.
...
All questions in this textbook are multiple-choice.
...
\item[A2] go, right, left, put, pick
\item[A3] step, right, left, put, pick
\item[A4] go, right, left, put, get
\end{enumerate}
\item What sensors does the robot have?
\begin{enumerate}
\item[A1] wall, home, north, gem, empty
\item[A2] wall, done, north, gem, empty 
\item[A3] wall, home, north, gem, full
\item[A4] clear, home, north, gem, empty 
\end{enumerate}
\item What can you do with a displayed Karel project that you clone into your account?.
\begin{enumerate}
\item[A1] Only open and view
\item[A2] Only run but not edit.
\item[A3] Only edit and run but not save
\item[A4] Edit, save, and run
\end{enumerate}
...
{\bf Objectives}: 
\begin{itemize}
\item Learn to operate the robot with the mouse.\\
\end{itemize}
...
{\bf Objectives}: 
\begin{itemize}
\item Type and run programs in input cells.
\item Add, clear, and remove input cells.
\item Learn to operate the robot via written commands.
\end{itemize}
...
\item Learn how to repeat tasks that involve repetitions.
\end{itemize}
...
\subsection{Algorithms, programs, and bugs}
