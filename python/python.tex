...
\subsection{Creating an account in NCLab}
...
\subsection{Cloning displayed projects vs. typing examples yourself}
...
In NCLab, you can type a Python code in the web browser, send it to a remote 
server where it gets processed, and after that the output is sent back to you 
and displayed in the web browser. Let's try this!

\subsection{Typing code and sending it to server}

Click into the input cell and type {\tt print "Hello, World!"}.
Then click on the "run" link under the input cell and your program will be sent to the server. 

\subsection{Receiving output}

The response should come back instantly, and it should be displayed 
...
\vspace{-1cm}
\newpage
\section{Using Python as calculator}

The Python input cell can be used as a calculator. 
...
and you can reuse them in your program many times.
Defining functions in Python is almost the same as defining new commands 
in Karel the Robot -- one uses the same keyword {\tt def}. Python functions 
can accept any number of variables as parameters. Usually, a function returns 
some result -- in that case one uses an optional {\tt return}. For example, 
the following simple function {\tt adds} adds two numbers and returns the result:

\begin{verbatim}
def add(a, b):
    return a + b
\end{verbatim}
Note the mandatory colon ':' at the end of the function declaration line.
