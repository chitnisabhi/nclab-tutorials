...
calculations using the Python interpreter, and provide the code.
\begin{enumerate}
\item 
$$
  987654321 - 123456789
...
def add(a, b):
    return a + b
\end{verbatim}
Note the mandatory colon ':' at the end of the function declaration line. The two lines of code above 
are a {\em function declaration} -- they do not produce any output and the function is not called. In order to 
call the function, one has to write additional code such as
...
8
\end{verbatim}
To mention another example, the following function does not return anything:

\begin{verbatim}
def say_hello():
    print "Hello!"
\end{verbatim}
Such a function is not very useful but it illustrates that the {\tt return} statement
does not have to be used. \\

\noindent
Let us stop here for the moment. What we know about functions now is good for some time, 
and we will learn more about them as we progress.
